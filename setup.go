...
func init() { plugin.Register("forward2", setup) }
...
func setup(c *caddy.Controller) error {
	...
	if f.Len() > max {
		return plugin.Error("forward2", fmt.Errorf("more than %d TOs configured: %d", max, f.Len()))
	}
...
func (f *Forward2) OnStartup() (err error) {
...
func (f *Forward2) OnShutdown() error {
	for _, p := range f.proxies {
		p.stop()
	}
	return nil
...
func parseForward(c *caddy.Controller) (*Forward2, error) {
	var (
		f   *Forward2
		err error
		i   int
...
func parseStanza(c *caddy.Controller) (*Forward2, error) {
	f := New()
...
func parseBlock(c *caddy.Controller, f *Forward2) error {
	switch c.Val() {
	case "except":
